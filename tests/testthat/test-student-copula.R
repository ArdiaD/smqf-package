...
test_that("f_student_copula_pdf is symmetric in u", {
  ...
  Sigma <- matrix(c(1, 0.5, 0.5, 1), 2, 2)
  u     <- c(0.3, 0.7)
  expect_equal(as.numeric(f_student_copula_pdf(u, mu, Sigma, nu = 5)),
               as.numeric(f_student_copula_pdf(rev(u), mu, Sigma, nu = 5)),
               tolerance = 1e-10)
})

test_that("f_student_copula_pdf: tail density higher than Gaussian copula at extreme quantile", {
  mu    <- c(0, 0)
  Sigma <- matrix(c(1, 0.5, 0.5, 1), 2, 2)
