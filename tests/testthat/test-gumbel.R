testthat::test_that("independence limit: CDF is product of marginals", {
  expect_equal(f_gumbel_copula_2d_cdf(c(0.7, 0.7), 1), 0.49, tolerance = 1e-14)
})

testthat::test_that("independence limit: PDF equals 1", {
  expect_equal(f_gumbel_copula_2d_pdf(c(0.7, 0.7), 1), 1, tolerance = 1e-14)
})

testthat::test_that("CDF is symmetric in its arguments", {
  u <- c(0.3, 0.8); theta <- 2
  expect_equal(
...
testthat::test_that("PDF is symmetric in its arguments", {
  u <- c(0.3, 0.8); theta <- 2
  expect_equal(
...
testthat::test_that("CDF does not exceed smaller marginal", {
  expect_true(f_gumbel_copula_2d_cdf(c(0.99, 0.99), 3) <= 0.99)
})

testthat::test_that("PDF is finite at extreme grid point", {
